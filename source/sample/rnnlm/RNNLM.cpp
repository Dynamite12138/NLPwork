 ...
 */
  ...
  * model (RNNLM). See more details about RNNLM in
...
#define MAX_LINE_LENGTH_HERE 1024 * 32
    ...
    char modelFN[MAX_NAME_LENGTH] = "";   // file name of the RNN model
    ...
    void LoadArgs(int argc, const char** argv, RNNModel& model);
    void Init(RNNModel& model);
    void Check(RNNModel& model);
    void Copy(RNNModel& tgt, RNNModel& src);
    void Clear(RNNModel& model, bool isNodeGrad);
    void InitModelTensor1D(XTensor& tensor, int num, RNNModel& model);
    void InitModelTensor2D(XTensor& tensor, int rowNum, int colNum, RNNModel& model);
    void Train(const char* train, bool isShuffled, RNNModel& model);
    void Update(RNNModel& model, RNNModel& grad, float epsilon, bool isNodeGrad);
    float GetProb(XTensor& output, XTensor& gold, XTensor* wordProbs = NULL);
    void Dump(const char* fn, RNNModel& model);
    void Read(const char* fn, RNNModel& model);
    void Test(const char* test, const char* result, RNNModel& model);
    int  LoadNGrams(FILE* file, int n, NGram* ngrams, int sentNum, int wordNum);
    int LoadOneSentence(FILE* file, NGram* ngrams);
    ...
    void Forward(XTensor inputs[], XTensor& output, RNNModel& model, RNNNet& net);
    void Backward(XTensor inputs[], XTensor& output, XTensor& gold, LOSS_FUNCTION_NAME loss,
        RNNModel& model, RNNModel& grad, RNNNet& net);
    void ForwardAutoDiff(XTensor inputs[], XTensor& output, RNNModel& model);
    void ForwardAutoDiff(NGram* ngrams, int batch, XTensor& hidden, XTensor& output, RNNModel& model);
    ...
    */
    int RNNLMMain(int argc, const char** argv)
    {
        if (argc == 0)
            return 1;
    ...
    */
    ...
    /* make a hard copy of the rnn model */
    void Copy(RNNModel& tgt, RNNModel& src)
    {
    ...
    */
    ...
    /*
    initialize a 1d tensor using the rnn model setting
    ...
    */
    ...
    /*
    initialize a 2d tensor using the rnn model setting
    >> tensor - the tensor to initialize
    ...
    */
    ...
    void Init(RNNModel& model)
    {
        ...
        for (int i = 0; i < model.hDepth; i++) {
            ...
            if (i == 0)
                InitModelTensor2D(model.hiddenW[i], model.eSize, model.hSize, model);
            else
    ...
    */
    void Train(const char* train, bool isShuffled, RNNModel& model)
    {
        ...
        float loss = 0;
        bool isEnd = false;
        bool learningRateHalfFalg = false;
        float formerProb = -1;

        NGram* ngrams = new NGram[MAX_N_GRAM];
        ...
        for (epoch = 0; epoch < nEpoch; epoch++) {
            ...
            while (ngramNum > 0) {
                ...
                ngramNum = LoadOneSentence(file, ngrams);
                if (ngramNum <= 0)
                    break;

                if(learningRateHalfFalg)
                    learningRate /= 2;
                /* previous n - 1 words */
                //XTensor* inputs = new XTensor[ngramNum];

                /* the predicted word */
                XTensor output;
                //InitTensor2D(&output, ngramNum, model.hSize, X_FLOAT, -1);

                /* the gold standard */
                XTensor* gold = new XTensor[ngramNum];
                ...
                //    //inputs[i].Dump(stderr, "input:" );
                //}
                //MakeWordBatch(inputs, ngrams, ngramNum, 0, model.vSize, model.devID);


                /* make the gold tensor */
                MakeWordBatch(gold, ngrams, ngramNum, 1, model.vSize, model.devID);
                ...
                if (!autoDiff) {
                    ...
                    /*for (int j = 0; j <= ngramNum - 1; j++)
                        gold[j].Dump(stderr, "gold");*/

                    /* this is implemented by gather function */
                    //ForwardAutoDiff(ngrams, ngramNum, output, model);
                    float probSum = 0; 
                    XTensor hidden;
                    InitTensor2D(&hidden, 1, model.eSize, X_FLOAT, -1);
                    //InitTensor2D(&output, batch, model.hSize, X_FLOAT, -1);
                    hidden.SetDataFixed(0.1f);
                    hidden.SetVarFlag();
                    for (int j = 0; j < ngramNum; j++) {
                        
                        ForwardAutoDiff(ngrams + j,ngramNum, hidden,output, model);
                        /* this is implemented by multiply function */
                        lossTensor = CrossEntropy(output, gold[j]);
                        //output.Dump(stderr, "output:");
                        //gold.Dump(stderr, "gold:");
                        //lossTensor.Dump(stderr, "lossTensor:");

                        /* automatic differentiation */
                        autoDiffer.Backward(lossTensor);
                        //lossTensor.Dump(stderr, "loss:");
    ...
    */
    int LoadOneSentence(FILE* file, NGram* ngrams)
    {
        ...
        while (pin > 0 || fgets(lineBuf, MAX_LINE_LENGTH_HERE - 1, file)) {
            ...
            //        break;
            //}
            //ngrams->words = new int[wordBufCount - 1];
            //memcpy(ngrams->words, wordBuf, sizeof(int) * (wordBufCount - 1));
            //num = wordBufCount - 1;
            //if (haveReadOneLine)
            //    break;
            /* set a finished flag if we reach the end of the sentence*/
            if (wordBufCount - 1 <= MAX_N_GRAM) {
                for (int j = 0; j < wordBufCount - 1; j++) {
                    memcpy(ngrams[j].words, wordBuf + pin + j, sizeof(int) );
                }
                num = wordBufCount - 1;
                pin = 0;
                wordBufCount = 0;
            ...
            /* record where to start next time if we break in the middle */
                ...
                for (int j = 0; j < MAX_N_GRAM; j++) {
                    memcpy(ngrams[j].words, wordBuf + pin + j, sizeof(int));
                }
                wordBufCount -= MAX_N_GRAM;
                num = MAX_N_GRAM;
                pin = pin + MAX_N_GRAM;
                break;
            }
    ...
    */
    ...
    >> ngramNum - batch size
    >> n - indicate which word is encode for each ngram 偏移量，0说明是input，1说明是gold
    ...
    */
    void MakeWordBatch(XTensor* batch, NGram* ngrams, int ngramNum, int n, int vSize, int devID)
    {
        ...
        //XTensor nowTensor, tensorSum;
        
        //InitTensor2D(&tensorSum, 0, ngramNum, X_FLOAT, -1);
        // 相当于把FNN的一列变成RNN的一行
        //int* rows = new int[ngramNum];
        //int* cols = new int[ngramNum];
        int rows[1], cols[1];
        for (int i = 0; i < ngramNum ; i++) {
            rows[0] = 0;
            cols[0] = ngrams->words[i + n];
            InitZeroOneTensor2D(batch[i], 1, vSize, rows, cols, 1, devID);
        }
        
        //for (int i = 0; i < ngramNum - n - 1; i++) {
        //    rows[i] = i;
        //    cols[i] = ngrams->words[i + n];
        //}
    ...
    */
    void ForwardAutoDiff(NGram* ngrams, int batch, XTensor& hidden, XTensor& output, RNNModel& model)
    {
        ...
        XTensor inputX;
        XTensor nowoutput;
        //TensorList smallTensor;
        XTensor outputSum;
        XTensor formerHidden;
        //XTensor hidden;
        //XTensor formerHidden;
        //int size = batch * (n - 1);
        ...
        //words.Dump(stderr, "words");

        //embeddingBig = Gather(model.embeddingW, words);
        //embeddingBig.Dump(stderr, "embeddingBig:");
        ...
        //InitTensor2D(&hidden, model.eSize, X_FLOAT, model.devID);
        //InitTensor2D(&hidden, 1, model.eSize, X_FLOAT, -1);
        //InitTensor2D(&hidden, 1, model.eSize, X_FLOAT, -1);
        ////InitTensor2D(&output, batch, model.hSize, X_FLOAT, -1);
        //hidden.SetDataFixed(0.1f);
        //hidden.SetVarFlag();
        //InitTensor2D(&outputSum, 0, model.vSize, X_FLOAT, -1);
        //outputSum.SetVarFlag();
        int temp[1];
        //for (int i = 0; i < batch; i++) {
            //XTensor hidden;
            ...
            //InitTensor2D(&hidden, 1, model.eSize, X_FLOAT, -1);
            //hidden.SetVarFlag();
        InitTensor1D(&words, 1, X_INT, model.devID);
        temp[0] = ngrams->words[0];
        words.SetData(temp, 1);
            //words.Dump(stderr, "words");

        inputW = Gather(model.embeddingW, words);
            //inputX.Dump(stderr, "inputX:");
            //hidden.Dump(stderr, "hidden:");

            /* hidden layers */
        for (int j = 0; j < depth; j++)
        {
            inputX = hidden + inputW;
            hidden = HardTanH(MMul(inputX, model.hiddenW[j]));
        }
                

            /* output layer */
            //output = LogSoftmax(MMul(hidden, model.outputW) + model.outputB, 1);
            //smallTensor.Add(Softmax(MMul(hidden, model.outputW), 1));
        output = Softmax(MMul(hidden, model.outputW), 1);
            //formerHidden = hidden;
            //smallTensor[i] = &Softmax(MMul(hidden, model.outputW), 1);
            //nowoutput.Dump(stderr, "output:");
            //outputSum.Dump(stderr, "outputSum:");


            //outputSum.Dump(stderr, "outputSum changed:");
        //}
        //output = outputSum;
    }
    ...
    */
    void Test(const char* test, const char* result, RNNModel& model)
    {
        ...
        while (ngramNum > 0) {
            ...
            if (ngramNum <= 0)
                break;
            TensorList outputlist(ngramNum);
            TensorList goldlist(ngramNum);
            /* previous n - 1 words */
            ...
            MakeWordBatch(gold, ngrams, ngramNum, 1, model.vSize, model.devID);
            if (!autoDiff) {
                ///* prepare an empty network for building the rnn */
                //RNNNet net;
            ...
            XTensor outputTensor, goldTensor;
            InitTensor2D(&outputTensor, ngramNum, model.vSize);
            Concatenate(outputlist, outputTensor, 0);
            InitTensor2D(&goldTensor, ngramNum, model.vSize);
            Concatenate(goldlist, goldTensor, 0);

            prob = GetProb(outputTensor, goldTensor, &probs);
            ///* get probabilities */
            //float prob = GetProb(output, gold, &probs);

            /* dump the test result */
            for (int i = 0; i < ngramNum; i++)
                fprintf(ofile, "%d ", ngrams->words[i]);
            for (int i = 0; i < ngramNum; i++)
                fprintf(ofile, "%d ", ngrams->words[i + 1]);
            fprintf(ofile, "||| ");
            for (int i = 0; i < ngramNum; i++)
                fprintf(ofile, "<s> ");
            for (int i = 0; i < ngramNum; i++)
